...
public class Permutation {
    ...
    public static List<List<Integer>> subsets(int[] array){
        ...
        return backtrack(0,array,new ArrayList<>(), finalList);
    }
    public static List<List<Integer>> backtrack(int s, int[] nums, List<Integer> list, List<List<Integer>> finalList){
        finalList.add(new ArrayList<>(list));
        for (int i = s; i < nums.length; i++) {
            list.add(nums[i]);
            backtrack(i+1, nums, list, finalList);
